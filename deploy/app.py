...
Deployment entry point (Render/HF Spaces). Same graph as langgraph_app.py, adapted to:
- build the Chroma index on first boot (the host doesn't ship a pre-built one)
- bind to 0.0.0.0 on $PORT so the platform's health check/proxy can reach it
"""
import os
